// KristoniumSafe.cpp by KHVirusser and N17Pro3426.
// Is just safe.
...
{
	if (MessageBoxW(NULL, L"This is the safe-harmless and GDI only version of Kristonium.exe This version doesn't have overwritting MBR(Master Boot Record) and crashing to show BSOD (Blue Screen of Death). Choose 'Yes' if you have an understanding this and agree to these terms.
Choosing also clicking 'No' nothing will happen.
\nThis is just safety version.", L"Follow me to Kristonium your machine", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"THIS IS THE LAST WARNING!!! Are you sure to continue? This is the safe version! You will try this on virtual machine or real PC. Choose 'Yes' to start it. Choose 'No' otherwise.", L"FINAL WARNING", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
